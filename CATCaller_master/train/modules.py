...
def attention(query, key, value, mask=None, dropout=None):
    ...
    d_k = query.size(-1)
    scores = torch.matmul(query, key.transpose(-2, -1)) / math.sqrt(d_k) #scores[batch,nhead,seq_len,seq_len]

    if mask is not None:  #mask[batch,1,1,seq_len]
...
class MultiBranch(nn.Module):
    ...
    def forward(self, query, key, value, key_padding_mask=None, incremental_state=None, need_weights=True, static_kv=False, attn_mask=None):
        ...
        :return:
        '''
        batch, seq_len, embed_size = query.size()
        ...
        for idx, embed_dim in enumerate(self.embed_dim_list):
            ...
            if branch_type == multiheadattention:  #multiattention forward
...
class FFN(nn.Module):
    ...
    def forward(self, x):
        ...
        output = self.w_2(x) #lx:can add dropout
...
class EncoderLayer(nn.Module):
    def __init__(self, index, d_model, d_ff, n_head, dropout=0.1):
        super(EncoderLayer, self).__init__()
        encoder_branch_type = ["attn:1:256:4", "dynamic:default:256:4"] #kernel:dim:head
        if encoder_branch_type == None:
            self.slf_attn = multiheadattention(h=n_head, d_model=d_model, dropout=dropout)
        else:
            layers = []
            embed_dims = []
            heads = []
            for layer_type in encoder_branch_type:
                embed_dims.append(int(layer_type.split(':')[2]))
                heads.append(int(layer_type.split(':')[3]))
                layers.append(self.get_layer(index, embed_dims[-1], heads[-1], layer_type, dropout))
            assert sum(embed_dims) == d_model
            self.slf_attn = MultiBranch(branches = layers, embed_dim_list= embed_dims)

        self.ffn = FFN(input_size=d_model, hidden_size=d_ff)
        self.layer_norm = nn.LayerNorm(d_model, eps=1e-6)
        self.norm_dropout = nn.Dropout(dropout)

    def get_layer(self, index, out_dim, num_heads, layer_type, dropout):
        kernel_size = layer_type.split(':')[1]
        if kernel_size == 'default':
            encoder_kernel_size_list = [3, 7, 15, 31, 31, 31] #need change
            kernel_size = encoder_kernel_size_list[index]
        else:
            kernel_size = int(kernel_size)
        padding_l = kernel_size // 2 if kernel_size % 2 == 1 else ((kernel_size - 1) // 2, kernel_size // 2)
        if 'lightweight' in layer_type:
            layer = LightweightConv(
                out_dim, kernel_size, padding_l=padding_l, weight_softmax=True,
                weight_dropout=0.1,
                num_heads=num_heads, with_linear=True)
        elif 'dynamic' in layer_type:
            layer = DynamicConv(
                out_dim, kernel_size, padding_l=padding_l, weight_softmax=True,
                num_heads=num_heads, weight_dropout=0.1, with_linear=True,
                glu=True,
            )
        elif 'attn' in layer_type:
            layer = multiheadattention(h=num_heads, d_model=out_dim, dropout=dropout)
        else:
    ...
    def forward(self, signal_emb, src_mask):
        ...
        enc_out, enc_self_attn = self.slf_attn(query=input_norm, key=input_norm, value=input_norm, key_padding_mask=src_mask.bool())  #change parameters for MultiBranch forward
