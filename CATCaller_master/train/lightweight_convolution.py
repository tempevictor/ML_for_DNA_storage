...
from train.utils import get_incremental_state, set_incremental_state, softmax
...
def LightweightConv(input_size, kernel_size=1, padding_l=None, num_heads=1,
    ...
    if torch.cuda.is_available():
        try:
            ...
            print(e)
            print("using LightweightConv1dTBC instead!")
...
class LightweightConv1dTBC(nn.Module):
    ...
    def _forward_unfolded(self, x, incremental_state):
        ...
        if self.weight_softmax:
            weight = softmax(weight, dim=1, onnx_trace=self.onnx_trace).type_as(weight)
